...
pedir_Factoria(ListaFactorias, NumFactoria):-
    ...
    ((NumFactoria >= 1, NumFactoria =< MaxFactorias, nth1(NumFactoria, ListaFactorias, Factoria), isEmpty(Factoria, Valor), Valor = 0, !);
...
rellenar_pared(FilaActual, LineasPatron, LineasPatronAux, LineasPatronOut, Pared, ParedAux, ParedOut,CajaIn, CajaOut):-
    ...
    rellenar_pared(FilaSiguiente, Resto, LineasPatronAux2, LineasPatronOut, RestoFilas, ParedAux2, ParedOut, CajaIn, CajaOut),!.

rellenar_pared(FilaActual, LineasPatron, LineasPatronAux, LineasPatronOut, Pared, ParedAux, ParedOut,CajaIn, CajaOut):-
    LineasPatron = [Primero|Resto], %Separación de la cabeza de las líneas de patrón de la cola de la misma
    isEmpty(Primero, Valor), %Verificar que la lista no está vacía
    ...
    FilaSiguiente is FilaActual+1, %Se obtiene la fila siguiente a estudiar
    rellenar_pared(FilaSiguiente, Resto, LineasPatronAux2, LineasPatronOut, RestoFilas, ParedAux2, ParedOut, Caja, CajaOut),!.
...
rellenar_pared(_,_, LineasPatronOut, LineasPatronOut, _, ParedOut, ParedOut, CajaOut, CajaOut).
...
realizar_llenado_paredes_jugadores(Situacion, NumJugadorInicial, NumJugador, NumJugadores, Supermatriz, SupermatrizOut):- %Situación en la que no se trabaja con el primer jugador de la ronda
   ...
   nth0(1, Lista_datos_jugador, Pared),
   nth0(2, Lista_datos_jugador, Suelo),
   rellenar_pared(1, LineasPatron, [], LineasPatronOut, Pared, [], ParedOut,Caja, CajaOut),
